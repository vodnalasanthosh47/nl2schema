...
def main():
    ...
    # open schemapile-pruned-sample201_to_700.json and filter out the failed schemas and create a new json with the remaining schemas
    # schemapile-pruned-sample201_to_700.json format is a schemas key with a list of schemas as value, we need to filter out the schemas with the indices in failed_schema_indices and save the remaining schemas to a new json file
    
    schemapile_json = get_json_from_file(
        f"{PATH_TO_DATA_FOLDER}/processed/schemapile-pruned-sample432_end.json"
    )

    schemas = schemapile_json["schemas"]

    filtered_schemas = [schema for i, schema in enumerate(schemas) if i + 1 not in failed_schema_indices]

    schemapile_json["schemas"] = filtered_schemas

    with open(f"{PATH_TO_DATA_FOLDER}/to_generate/schemapile-pruned-sample432_end_filtered.json", "w") as f:
        json.dump(schemapile_json, f)
