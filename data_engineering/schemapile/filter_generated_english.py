...
def main():
    ...
    # Find the indices of the schemas that are detected as English from pruned file

    schemapile = get_json_from_file(
        f"{PATH_TO_DATA_FOLDER}/processed/schemapile-pruned-sample201_to_700.json"
    )
    ...
    # open schemapile_200.json and filter out the failed schemas and create a new json with the remaining schemas
    # schemapile_200.json format is a list of dicts with "input" and "output" keys, where "input" is the natural language description and "output" is the JSON schema map
    with open(f"{PATH_TO_DATA_FOLDER}/generated/schemapile-pruned-sample201_to_700-with-nl.json", "r") as f:
        data = json.load(f)

    # Filter out failed schemas
    filtered_data = [item for i, item in enumerate(data) if i + 1 not in failed_schema_indices]

    # Save the filtered data to a new JSON file
    with open(f"{PATH_TO_DATA_FOLDER}/training/schemapile201_to_700_filtered.json", "w") as f:
        json.dump(filtered_data, f)

    print("Filtered data saved to schemapile201_to_700_filtered.json")
